...
export class Engenheiro extends Funcionario{
    ...
    constructor (nome: string, matricula: string, salario: number, telefone: string, endereco: string, dataNascimento: string, numCrea: number)
    ...
    public set setNumCrea(novoNumCrea: number){
        if(novoNumCrea===0){
            console.log("\n ERRO: O número do crea não pode ser vazio!");
            return;
        }
